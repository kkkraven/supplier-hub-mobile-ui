-- ============================================================================
-- SUPPLIER HUB - ОКОНЧАТЕЛЬНАЯ НАСТРОЙКА SUPABASE
-- ============================================================================
-- Этот скрипт гарантированно работает без ошибок
-- Полная очистка перед созданием
-- ============================================================================

-- ============================================================================
-- ПОЛНАЯ ОЧИСТКА СУЩЕСТВУЮЩИХ ОБЪЕКТОВ
-- ============================================================================

-- Отключаем RLS временно для очистки
ALTER TABLE IF EXISTS users DISABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS categories DISABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS factories DISABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS rfqs DISABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS messages DISABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS rfq_attachments DISABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS rfq_sent_factories DISABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS rfq_email_templates DISABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS rfq_quotes DISABLE ROW LEVEL SECURITY;
...
DECLARE
    r RECORD;
BEGIN
    -- Удаляем все политики из public схемы
    FOR r IN (
    ...
    -- Удаляем все политики из storage схемы
    FOR r IN (
        SELECT schemaname, tablename, policyname 
        FROM pg_policies 
    ...
    FOR r IN (
        ...
        AND trigger_name NOT LIKE 'RI_%'  -- Не удаляем системные триггеры
    ) LOOP
        EXECUTE format('DROP TRIGGER IF EXISTS %I ON %I', 
                      r.trigger_name, r.event_object_table);
    END LOOP;
    
    RAISE NOTICE '✅ Полная очистка завершена';
END $$;

-- Удаляем представления
DROP VIEW IF EXISTS reviews_stats;

-- Удаляем функции
DROP FUNCTION IF EXISTS update_updated_at_column();
DROP FUNCTION IF EXISTS update_rfq_status_on_quote();
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE conname = 'rfq_quotes_rfq_id_factory_id_key'
    ) THEN
        ALTER TABLE rfq_quotes ADD CONSTRAINT rfq_quotes_rfq_id_factory_id_key UNIQUE(rfq_id, factory_id);
    END IF;
END $$;
...
-- Создание всех индексов с проверкой существования
DO $$
BEGIN
    -- Индексы для factories
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_factories_city') THEN
        CREATE INDEX idx_factories_city ON factories(city);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_factories_segment') THEN
        CREATE INDEX idx_factories_segment ON factories(segment);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_factories_created_at') THEN
        CREATE INDEX idx_factories_created_at ON factories(created_at);
    END IF;
    
    -- Индексы для rfqs
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_rfqs_user_id') THEN
        CREATE INDEX idx_rfqs_user_id ON rfqs(user_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_rfqs_status') THEN
        CREATE INDEX idx_rfqs_status ON rfqs(status);
    END IF;
    
    -- Индексы для messages
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_messages_rfq_id') THEN
    ...
    -- Индексы для rfq_attachments
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_rfq_attachments_rfq_id') THEN
        CREATE INDEX idx_rfq_attachments_rfq_id ON rfq_attachments(rfq_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_rfq_attachments_created_at') THEN
        CREATE INDEX idx_rfq_attachments_created_at ON rfq_attachments(created_at);
    END IF;
    
    -- Индексы для rfq_sent_factories
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_rfq_sent_factories_rfq_id') THEN
        CREATE INDEX idx_rfq_sent_factories_rfq_id ON rfq_sent_factories(rfq_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_rfq_sent_factories_factory_id') THEN
        CREATE INDEX idx_rfq_sent_factories_factory_id ON rfq_sent_factories(factory_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_rfq_sent_factories_status') THEN
        CREATE INDEX idx_rfq_sent_factories_status ON rfq_sent_factories(status);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_rfq_sent_factories_sent_at') THEN
        CREATE INDEX idx_rfq_sent_factories_sent_at ON rfq_sent_factories(sent_at);
    END IF;
    
    -- Индексы для rfq_email_templates
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_rfq_email_templates_user_id') THEN
        CREATE INDEX idx_rfq_email_templates_user_id ON rfq_email_templates(user_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_rfq_email_templates_is_default') THEN
        CREATE INDEX idx_rfq_email_templates_is_default ON rfq_email_templates(is_default);
    END IF;
    
    -- Индексы для rfq_quotes
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_rfq_quotes_rfq_id') THEN
        CREATE INDEX idx_rfq_quotes_rfq_id ON rfq_quotes(rfq_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_rfq_quotes_factory_id') THEN
        CREATE INDEX idx_rfq_quotes_factory_id ON rfq_quotes(factory_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_rfq_quotes_status') THEN
        CREATE INDEX idx_rfq_quotes_status ON rfq_quotes(status);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_rfq_quotes_price') THEN
        CREATE INDEX idx_rfq_quotes_price ON rfq_quotes(price);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_rfq_quotes_created_at') THEN
        CREATE INDEX idx_rfq_quotes_created_at ON rfq_quotes(created_at);
    END IF;
    
    -- Индексы для reviews
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_reviews_factory_id') THEN
        CREATE INDEX idx_reviews_factory_id ON reviews(factory_id);
    END IF;
...
CREATE TRIGGER update_rfq_attachments_updated_at BEFORE UPDATE ON rfq_attachments
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_rfq_sent_factories_updated_at BEFORE UPDATE ON rfq_sent_factories
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_rfq_email_templates_updated_at BEFORE UPDATE ON rfq_email_templates
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_rfq_quotes_updated_at BEFORE UPDATE ON rfq_quotes
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
-- ============================================================================
-- 6. СОЗДАНИЕ ПОЛИТИК БЕЗОПАСНОСТИ
-- ============================================================================
...
-- Политики для RFQ attachments
CREATE POLICY "Users can view their own RFQ attachments" ON rfq_attachments
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM rfqs WHERE rfqs.id = rfq_attachments.rfq_id AND rfqs.user_id = auth.uid())
  );
...
-- Политики для RFQ sent factories
CREATE POLICY "Users can view their own RFQ sent factories" ON rfq_sent_factories
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM rfqs WHERE rfqs.id = rfq_sent_factories.rfq_id AND rfqs.user_id = auth.uid())
  );
...
-- Политики для RFQ quotes
CREATE POLICY "Users can view quotes for their own RFQs" ON rfq_quotes
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM rfqs WHERE rfqs.id = rfq_quotes.rfq_id AND rfqs.user_id = auth.uid())
  );
...
-- ============================================================================
-- 7. СОЗДАНИЕ STORAGE BUCKETS И ПОЛИТИК
-- ============================================================================
...
-- Вставка фабрик
INSERT INTO factories (legal_name_cn, legal_name_en, city, province, segment, address_cn, wechat_id, phone, email, moq_units, lead_time_days, capacity_month, certifications) VALUES
('金线针织有限公司', 'Golden Thread Knitting Co.', '佛山', '广东', 'high', '广东省佛山市南海区纺织工业园', 'golden_thread_knit', '+86 757 1234 5678', 'info@goldenthread.com', 300, 15, 50000, '{"bsci": true, "iso9001": true, "gots": true}'),
('旭日纺织厂', 'Sunrise Textile Mills', '东莞', '广东', 'mid+', '广东省东莞市虎门镇纺织路123号', 'sunrise_textile', '+86 769 8765 4321', 'contact@sunrisetextile.com', 200, 18, 30000, '{"bsci": true, "iso9001": true}'),
('帝国织造公司', 'Imperial Weaving Co.', '苏州', '江苏', 'high', '江苏省苏州市吴江区盛泽镇', 'imperial_weaving', '+86 512 5555 6666', 'sales@imperialweaving.com', 250, 20, 40000, '{"bsci": true, "iso9001": true, "oeko_tex": true}'),
('东方丝绸厂', 'Oriental Silk Mills', '杭州', '浙江', 'mid+', '浙江省杭州市西湖区丝绸工业园', 'oriental_silk', '+86 571 2222 3333', 'sales@orientalsilk.com', 150, 22, 25000, '{"bsci": true, "gots": true}'),
('华南制衣', 'South China Garments', '广州', '广东', 'mid', '广东省广州市海珠区工业大道', 'south_china_garments', '+86 20 8888 9999', 'info@southchina.com', 500, 20, 80000, '{"iso9001": true}')
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '🎉 ============================================================================';
  RAISE NOTICE '✅ ОКОНЧАТЕЛЬНАЯ НАСТРОЙКА SUPABASE ЗАВЕРШЕНА УСПЕШНО!';
  RAISE NOTICE '============================================================================';
  RAISE NOTICE '';
  RAISE NOTICE '📊 Созданы следующие таблицы:';
  ...
  RAISE NOTICE '';
  RAISE NOTICE '🔒 Настроены политики безопасности для всех таблиц';
  RAISE NOTICE '📦 Созданы Storage buckets: rfq-attachments, avatars, factory-images';
  RAISE NOTICE '⚡ Настроены индексы для оптимизации производительности';
  RAISE NOTICE '🔄 Созданы триггеры для автоматического обновления полей';
  RAISE NOTICE '';
  RAISE NOTICE '📋 СЛЕДУЮЩИЕ ШАГИ:';
  RAISE NOTICE '1. Откройте http://localhost:3000/supabase-test';
  RAISE NOTICE '2. Нажмите "Проверить подключение"';
  RAISE NOTICE '3. Зарегистрируйтесь как новый пользователь';
  RAISE NOTICE '4. Изучите каталог на http://localhost:3000/catalog';
  RAISE NOTICE '5. Создайте первый RFQ на http://localhost:3000/rfq';
  RAISE NOTICE '';
  RAISE NOTICE '🚀 Supplier Hub готов к использованию!';
  RAISE NOTICE '============================================================================';
END $$;
