-- ============================================================================
-- SUPPLIER HUB - БЕЗОПАСНАЯ НАСТРОЙКА SUPABASE
-- ============================================================================
-- Этот скрипт безопасен для повторного выполнения
-- Все команды содержат проверки существования
-- ============================================================================

-- Очистка существующих политик и триггеров перед созданием новых
DO $$ 
DECLARE
    r RECORD;
BEGIN
    ...
    FOR r IN (
        ...
        AND (trigger_name LIKE 'update_%_updated_at' OR trigger_name LIKE 'update_%_trigger')
    ) LOOP
        EXECUTE format('DROP TRIGGER IF EXISTS %I ON %I', 
                      r.trigger_name, r.event_object_table);
    END LOOP;
    
    -- Дополнительно удаляем специфичные триггеры
    EXECUTE 'DROP TRIGGER IF EXISTS update_rfq_status_on_quote_trigger ON rfq_quotes';
    
    RAISE NOTICE '✅ Очистка существующих политик и триггеров завершена';
END $$;
...
-- Добавляем уникальное ограничение для предотвращения дублирования предложений
DO $$
BEGIN
    IF NOT EXISTS (
...
SELECT create_index_if_not_exists('idx_factories_city', 'factories', '(city)');
SELECT create_index_if_not_exists('idx_factories_segment', 'factories', '(segment)');
SELECT create_index_if_not_exists('idx_factories_created_at', 'factories', '(created_at)');
SELECT create_index_if_not_exists('idx_rfqs_user_id', 'rfqs', '(user_id)');
SELECT create_index_if_not_exists('idx_rfqs_status', 'rfqs', '(status)');
SELECT create_index_if_not_exists('idx_messages_rfq_id', 'messages', '(rfq_id)');
SELECT create_index_if_not_exists('idx_messages_sender_id', 'messages', '(sender_id)');
SELECT create_index_if_not_exists('idx_messages_receiver_id', 'messages', '(receiver_id)');
SELECT create_index_if_not_exists('idx_rfq_attachments_rfq_id', 'rfq_attachments', '(rfq_id)');
SELECT create_index_if_not_exists('idx_rfq_attachments_created_at', 'rfq_attachments', '(created_at)');
SELECT create_index_if_not_exists('idx_rfq_sent_factories_rfq_id', 'rfq_sent_factories', '(rfq_id)');
SELECT create_index_if_not_exists('idx_rfq_sent_factories_factory_id', 'rfq_sent_factories', '(factory_id)');
SELECT create_index_if_not_exists('idx_rfq_sent_factories_status', 'rfq_sent_factories', '(status)');
SELECT create_index_if_not_exists('idx_rfq_sent_factories_sent_at', 'rfq_sent_factories', '(sent_at)');
SELECT create_index_if_not_exists('idx_rfq_email_templates_user_id', 'rfq_email_templates', '(user_id)');
SELECT create_index_if_not_exists('idx_rfq_email_templates_is_default', 'rfq_email_templates', '(is_default)');
SELECT create_index_if_not_exists('idx_rfq_quotes_rfq_id', 'rfq_quotes', '(rfq_id)');
SELECT create_index_if_not_exists('idx_rfq_quotes_factory_id', 'rfq_quotes', '(factory_id)');
SELECT create_index_if_not_exists('idx_rfq_quotes_status', 'rfq_quotes', '(status)');
SELECT create_index_if_not_exists('idx_rfq_quotes_price', 'rfq_quotes', '(price)');
SELECT create_index_if_not_exists('idx_rfq_quotes_created_at', 'rfq_quotes', '(created_at)');
SELECT create_index_if_not_exists('idx_reviews_factory_id', 'reviews', '(factory_id)');
SELECT create_index_if_not_exists('idx_reviews_status', 'reviews', '(status)');
SELECT create_index_if_not_exists('idx_reviews_created_at', 'reviews', '(created_at DESC)');
SELECT create_index_if_not_exists('idx_reviews_rating', 'reviews', '(rating)');
SELECT create_index_if_not_exists('idx_reviews_user_id', 'reviews', '(user_id)');
...
-- Создание всех триггеров (безопасно)
...
-- Политики для Storage (создаем только если не существуют)
DO $$
BEGIN
    -- Политики для rfq-attachments
    IF NOT EXISTS (
        ...
        AND policyname = 'Users can upload RFQ attachments'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE schemaname = 'storage' AND tablename = 'objects' 
        AND policyname = 'Users can view their own RFQ attachments'
    ) THEN
    ...
    -- Политики для factory-images
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE schemaname = 'storage' AND tablename = 'objects' 
        AND policyname = 'Anyone can view factory images'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE schemaname = 'storage' AND tablename = 'objects' 
        AND policyname = 'Admins can upload factory images'
    ) THEN
